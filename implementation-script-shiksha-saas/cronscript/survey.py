...
def checkIfObsMappedToProgram(accessToken, obsExt, parentFolder):
    ...
    if responseSearchSol.status_code == 200:
        ...
        for eachSol in responseSearchSol['result']['data']:
            ...
            print(responseSolDump,"responseSolDump")
...
def getRolesAndTenantIdAndOrgIdFromUserToken(decodedToken):
        ...
        orgIDFromTemplate = clean_single_value(decodedToken['data']['organization_codes'][0])
        print(orgIDFromTemplate,"orgIDFromTemplate12")
...
def generateAccessToken(solutionName_for_folder_path):
    ...
    # sys.exit()
    print(config.get(environment, 'keyclockAPIUrl'),'login')
    print(config.get(environment, 'userLoginHost'),'login2')
    print(loginBody,'loginBody')
    print(config.get(environment, 'userLoginHost') + config.get(environment, 'keyclockapiurl'),'url**')
...
def createChild(solutionName_for_folder_path, observationExternalId, accessToken):
    ...
    print(headersSol_prog_mapping,'headersSol_prog_mapping')
            ...
            for row_index_env in range(2, detailsEnvSheet.nrows):
                ...
                if dictDetailsEnv["survey_start_date"]:
                    ...
                    if dictDetailsEnv["survey_end_date"]:
                        ...
                        responseCreateSolutionApikson = responseCreateSolutionApi.json()
                        solutionID = responseCreateSolutionApikson['result']['solutionId']
                        print(solutionID)
                    ...
                    # bodySolutionUpdate = {
                     #     "scope": {"entityType": scopeEntityType, "entities": scopeEntities, "roles": scopeRoles}}
                        ...
                        #     "scope": {"entityType": scopeEntityType, "entities": scopeEntities, "roles": scopeRoles}}
                          ...
                          print(solutionUpdate,"solutionUpdate")
            ...
            for row_index_env in range(2, detailsEnvSheet.nrows):
                ...
                if typeOfCertificate == 'One Logo - One Signature':
                   ...
                   signature1 = gdown.download(file_url, dest_file, quiet=False)
                    ...
                    print("--->Logos and signature downlading are failed(check if drive link are  Anyon with the link or not)<---")
