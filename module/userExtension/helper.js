    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (let index = userRolesCSVData.length - 1; index >= 0; index--) {
          ...
          if (
            programOperation === messageConstants.common.REMOVE_OPERATION ||
            programOperation === messageConstants.common.ADD_OPERATION
          ) {
        ...
        outerloop: for (const csvRow of userRolesCSVData) {
          ...
          try {
            ...
            if (!existingUser) {
              if (
                [
                  messageConstants.common.OVERRIDE_OPERATION,
                  messageConstants.common.ADD_OPERATION,
                  messageConstants.common.APPEND_OPERATION,
              ...
              if (userRole.programOperation && userRole.programs) {
                for (const program of userRole.programs) {
                  ...
                  if (userRole.programOperation === messageConstants.common.OVERRIDE_OPERATION) {
                    ...
                    userRole.programOperation === messageConstants.common.ADD_OPERATION ||
                    userRole.programOperation === messageConstants.common.APPEND_OPERATION
                  ) {
                    if (currentRoleInfoIndex !== -1) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          return resolve({
            success: true,
            message: messageConstants.apiResponses.SKIPPING_EVENT_PROCESSING,
          });
        }
        ...
        if (affectedProgramIds.size > 0) {
          const programDocs = await programsQueries.programDocuments({ _id: { $in: Array.from(affectedProgramIds) } }, [
            'name',
            'externalId',
          ]);
        ...
        for (const programEntry of userExtensionDoc.programRoleMapping || []) {
          ...
          for (const removedRole of rolesRemoved) {
            ...
            kafkaMessages.push({
              ...
              entity: messageConstants.common.PROGRAM_CHECK,
              meta: {
                programInformation: {
                  id: programId,
        ...
        if (JSON.stringify(updatedProgramRoleMapping) !== JSON.stringify(userExtensionDoc.programRoleMapping)) {
          await userExtensionsQueries.updateUserExtensionDocument(
        ...
        for (const kafkaMessage of kafkaMessages) {
          let check = await kafkaClient.pushProgramOperationEvent(kafkaMessage);
        }
        ...
        return resolve({
          ...
          message: kafkaMessages.length
            ...
            : messageConstants.apiResponses.NO_CHANGES_IN_PROGRAM_MAPPING,
        });
      } catch (error) {
        return reject(error);
      }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (deleteResult.deletedCount > 0) {
          return resolve({
            success: true,
            message: messageConstants.apiResponses.USER_EXTENSION_DELETED,
          });
        } else {
          return resolve({
            success: true,
...
function addNewProgramEntry(
  ...
  for (const role of newRoles) {
    kafkaEventPayloads.push(
      createKafkaPayload(userProfile, programId, role, messageConstants.common.CREATE_EVENT_TYPE, programInfoMap)
    );
  }
}
